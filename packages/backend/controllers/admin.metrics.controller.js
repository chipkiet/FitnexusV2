...
export async function getOverviewMetrics(_req,res){
  try{
    ...
    const activeUsers = await db.User.count({ where:{ lastActiveAt:{ [Op.gte]: activeSince }}});
    ...
    const y0 = new Date(today); y0.setDate(y0.getDate()-1);
    const wPrev0 = new Date(w0); wPrev0.setDate(wPrev0.getDate()-7);
    const mPrev0 = new Date(m0); mPrev0.setMonth(mPrev0.getMonth()-1);
    ...
    const [
      ...
      db.User.count({ where: { lastActiveAt: { [Op.gte]: new Date(Date.now()-10*60*1000), [Op.lt]: new Date(Date.now()-5*60*1000) } } }),
      // revenues previous day/week/month
      sum({ status:'completed', created_at: { [Op.gte]: y0, [Op.lt]: today } }),
      sum({ status:'completed', created_at: { [Op.gte]: wPrev0, [Op.lt]: w0 } }),
      sum({ status:'completed', created_at: { [Op.gte]: mPrev0, [Op.lt]: m0 } }),
    ]);

    const pct = (cur, prev) => (prev && prev !== 0 ? Math.round(((Number(cur||0)-Number(prev||0))/Number(prev))*100) : null);

    // New users by day (last 14d)
    const days=14; const from=new Date(); from.setDate(from.getDate()-(days-1)); from.setHours(0,0,0,0);
    let newRows=[];
    if(dialect==='postgres'){
    ...
    const mapN=new Map(newRows.map(r=>[String(r.day), Number(r.count)||0]));
    const new_users_by_day=[]; for(let i=0;i<days;i++){ const d=new Date(from); d.setDate(from.getDate()+i); const k=fmtD(d); new_users_by_day.push({ date:k, count: mapN.get(k)||0 }); }

    // Revenue by month (last 6m)
    const months=6; const start6=sMonth(addM(new Date(),-(months-1)));
    let revRows=[];
    if(dialect==='postgres'){
      const [rows]=await sequelize.query(
    ...
    const mapR=new Map(revRows.map(r=>[String(r.ym), Number(r.revenue)||0]));
    const revenue_by_month=[]; for(let i=0;i<months;i++){ const d=sMonth(addM(start6,i)); const k=fmtM(d); revenue_by_month.push({ month:k, revenue: mapR.get(k)||0 }); }

    // Simple 1-month cohort retention for last 4 cohorts
    const cohorts=4; const first=sMonth(addM(new Date(),-cohorts)); const retention=[];
    for(let i=0;i<cohorts;i++){
      const c0=sMonth(addM(first,i)); const c1=sMonth(addM(c0,1)); const n1=sMonth(addM(c1,1));
      const newUsers = await db.User.count({ where:{ created_at:{ [Op.gte]: c0, [Op.lt]: c1 }}});
      let retained=0; if(newUsers>0){
        const ids = await db.User.findAll({ where:{ created_at:{ [Op.gte]: c0, [Op.lt]: c1 }}, attributes:['user_id'], raw:true });
        const arr=ids.map(u=>u.user_id);
        retained = arr.length? await db.User.count({ where:{ user_id:{ [Op.in]: arr }, lastActiveAt:{ [Op.gte]: c1, [Op.lt]: n1 }}}):0;
      }
      retention.push({ cohort: fmtM(c0), new_users: newUsers, retained_next_month: retained, retention_rate: newUsers? Math.round(retained*100/newUsers):0 });
    }

    // Alerts
    const now=new Date(); const pendingOld=new Date(now.getTime()-86400000);
    const [pendingCount, lockedCount, upcomingExp] = await Promise.all([
      db.Transaction.count({ where:{ status:'pending' }}),
      db.User.count({ where:{ isLocked:true }}),
      db.User.count({ where:{ user_type:'premium', user_exp_date:{ [Op.gte]: now, [Op.lte]: new Date(now.getTime()+7*86400000) }}}),
    ]);
    const pendingTop = await db.Transaction.findAll({ where:{ status:'pending', created_at:{ [Op.lte]: pendingOld }}, attributes:['transaction_id','amount','created_at'], include:[{ model: db.User, as:'userTransaction', attributes:['user_id','email','username'] }], order:[['created_at','ASC']], limit:3 });

    res.json({ success:true, data:{
      cards:{
        users_total: totalUsers,
        users_total_prev_week: Number(usersPrevWeekTotal||0),
        users_total_delta: Number(totalUsers) - Number(usersPrevWeekTotal||0),
        ...
        users_active_5m_delta: Number(activeUsers||0) - Number(activeUsersPrevWindow||0),
        ...
        revenue_today_delta: Number(revToday||0) - Number(revYesterday||0),
        ...
        revenue_month_delta: Number(revMonth||0) - Number(revPrevMonth||0),
        revenue_month_pct: pct(revMonth||0, revPrevMonth||0),
      },
      charts:{ new_users_by_day, revenue_by_month, retention_cohort_1m: retention },
      alerts:{ pending_transactions:{ count: pendingCount, top: pendingTop }, locked_users:{ count: lockedCount }, upcoming_premium_expiry_7d:{ count: upcomingExp } }
    }});
  }catch(err){
