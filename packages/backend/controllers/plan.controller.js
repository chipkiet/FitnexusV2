...
export async function addExerciseToPlan(req, res) {
  try {
    ...
    if (!Number.isFinite(exercise_id) || exercise_id <= 0) {
      return res
        .status(422)
        .json({ success: false, message: "exercise_id is required" });
    }
    const exercise = await Exercise.findByPk(exercise_id);
    if (!exercise) {
      return res
        .status(422)
        .json({ success: false, message: "exercise_id not found" });
    }
    ...
    const sets_recommended = Number.isFinite(
      parseInt(req.body?.sets_recommended, 10)
    )
      ? parseInt(req.body?.sets_recommended, 10)
      : null;
    const reps_recommended =
      req.body?.reps_recommended != null
        ? String(req.body.reps_recommended)
        : null;
    const rest_period_seconds = Number.isFinite(
      parseInt(req.body?.rest_period_seconds, 10)
    )
      ? parseInt(req.body?.rest_period_seconds, 10)
      : null;
...
export async function listMyPlans(req, res) {
  try {
    ...
    if (mine === "1" || mine.toLowerCase() === "true") {
      ...
      return res
        .status(200)
        .json({ success: true, data: { items, total: items.length } });
    }
...
export async function reorderPlanExercises(req, res) {
  ...
  try {
    ...
    if (!Array.isArray(updates) || updates.length === 0) {
      ...
      return res
        .status(422)
        .json({ success: false, message: "exercises array required" });
    }
    ...
    for (const item of updates) {
      ...
      if (
        !Number.isFinite(plan_exercise_id) ||
        !Number.isFinite(session_order)
      ) {
        await t.rollback();
...
export async function updatePlanExercise(req, res) {
  try {
    ...
    if (req.body?.sets_recommended !== undefined) {
      ...
      updateData.sets_recommended =
        Number.isFinite(sets) && sets > 0 ? sets : null;
    }

    if (req.body?.reps_recommended !== undefined) {
      updateData.reps_recommended =
        req.body.reps_recommended != null
    ...
    if (req.body?.rest_period_seconds !== undefined) {
      ...
      updateData.rest_period_seconds =
