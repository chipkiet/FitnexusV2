...
const MEAL_TYPES = [
  { key: "breakfast", label: "Sáng", icon: "/food-icon/Sunrise Scribby.png" },
  { key: "lunch", label: "Trưa", icon: "/food-icon/Sun Doodle.png" },
  { key: "dinner", label: "Tối", icon: "/food-icon/Night Doodle.png" },
  {
    key: "snack",
    label: "Snack",
    icon: "/food-icon/Potato Chips Pulsar Color.png",
  },
];

const FOOD_ICONS = {
  chicken: "/food-icon/Fried Chicken Scribby.png",
  breast: "/food-icon/Fried Chicken Scribby.png", // Dùng chung ảnh gà
  salmon: "/food-icon/Salmon.png",
  tuna: "/food-icon/Salmon.png",
  beef: "/food-icon/Beef Scribby.png",
  egg: "/food-icon/Sunny Side Up Eggs Doodle.png",
  milk: "/food-icon/Milk Bottle Scribby.png",
  rice: "/food-icon/Rice Bowl Doodle.png",
  oat: "/food-icon/Oat Flour.png",
  oatmeal: "/food-icon/Porridge Doodle.png", // Dùng chung ảnh yến mạch
  bread: "/food-icon/Bread Loaf Doodle.png",
  potato: "/food-icon/Potato Doodle.png",
  sweet: "/food-icon/Sweet Potato Scribby.png", // Dùng chung ảnh khoai
  yogurt: "/food-icon/Yogurt Scribby.png",
  banana: "/food-icon/Banana Scribby.png",
  apple: "/food-icon/Apple Fruit Doodle.png",
  avocado: "/food-icon/Avocado Scribby.png",
  broccoli: "/food-icon/Broccoli Doodle.png",
  spinach: "/food-icon/Spinach Doodle.png",
  almond: "/food-icon/Almond.png",
  tofu: "/food-icon/Soy Scribby.png",
  protein: "/food-icon/Protein Supplement Doodle.png",
  shake: "/food-icon/Milkshake Doodle.png", // Dùng chung ảnh bột protein
};
const getFoodIcon = (name = "") => {
  const lc = name.toLowerCase();
  for (const [k, v] of Object.entries(FOOD_ICONS)) {
    if (lc.includes(k)) return v;
  }
  // Trả về một ảnh mặc định (bạn chọn 1 ảnh chung chung trong thư mục của bạn)
  return "/food-icon/Protein Supplement Doodle.png";
};
...
function WeekStrip({ selected, onChange, datesWithMeals = new Set() }) {
  ...
  return (
    ...
    >
      ...
      <div
        style={{ flex: 1, display: "flex", gap: 4, justifyContent: "center" }}
      >
        {days.map((d) => {
  ...
  return (
    <div
      style={{
        ...
        padding: "10px 6px",
        borderRadius: 12,
        background: accent ? "#EEF2FF" : "#F8FAFC",
...
export default function MealPlanner() {
  ...
  useEffect(() => {
    axios
      .get(`${API}/api/foods/popular`)
      .then((r) => {
        setPopularFoods(r.data);
        setFoods(r.data);
      })
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setSearching(true);
      try {
        const r = await axios.get(`${API}/api/foods/search`, {
  ...
  const loadLog = useCallback(() => {
    ...
    axios
      .get(`${API}/api/meals/${userId}/summary${q}`)
      .then((r) => {
        setLog(r.data.meals || {});
  ...
  return (
    ...
    >
      ...
      <main
        style={{ paddingTop: 80, paddingBottom: 64, padding: "80px 20px 64px" }}
      >
                    ...
                    {foods.map((f) => (
                      <motion.div
                        ...
                        style={{
                          ...
                          cursor: "pointer",
                          background:
                            selectedFood?.id === f.id ? "#ECFDF5" : "#fff",
                          transition: "all 0.15s",
                        }}
                      >
                        <img
                          src={getFoodIcon(f.name)}
                          alt={f.name}
                          style={{
                            width: 36,
                            height: 36,
                ...
                <div style={{ padding: 16 }}>
                  ...
                  <AnimatePresence mode="wait">
                    {selectedFood ? (
                      ...
                      >
                        ...
                        >
                          <img
                            src={getFoodIcon(selectedFood.name)}
                        ...
                        {preview && (
                          ...
                          >
                            {quantity}g → {preview.calories} kcal · P:{" "}
                            {preview.protein}g · C: {preview.carbs}g · F:{" "}
                            {preview.fat}g
                          </div>
                        )}
                      </motion.div>
                      ...
                      {MEAL_TYPES.map((m) => (
                        <button
                          ...
                          style={{
                            flex: 1,
                            padding: "7px 0",
                            border: `1px solid ${mealType === m.key ? "#6366F1" : "#E2E8F0"}`,
                            borderRadius: 8,
                            fontSize: 11,
                            fontWeight: 700,
                            cursor: "pointer",
                  ...
                  <div style={{ marginBottom: 14 }}>
                    ...
                    <input
                      ...
                      min={1}
                      onChange={(e) =>
                        setQuantity(parseFloat(e.target.value) || 100)
                      }
                      style={{
                        width: "100%",
                        padding: "8px 10px",
                    ...
                    >
                      📅 Lưu vào ngày{" "}
                      <strong style={{ color: "#10B981" }}>{activeDate}</strong>
                    </div>
                  </div>
                  ...
                  <motion.button
                    ...
                    style={{
                      ...
                      background: selectedFood ? "#10B981" : "#D1FAE5",
                      color: "#fff",
                      fontFamily: "inherit",
                      transition: "all 0.2s",
                            ...
                            >
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                <div style={{ fontSize: 10, color: "#94A3B8" }}>
                                  {m.quantity_g}g · {m.calories} kcal
                                </div>
                              </div>
                              <button
                                onClick={() => openEdit(m)}
              ...
              >
                ...
                <div>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    {MEAL_TYPES.map((m) => (
                      <button
                        key={m.key}
                        onClick={() => setEditType(m.key)}
                        style={{
                          flex: 1,
                          padding: "7px 4px",
                          border: `1px solid ${editType === m.key ? "#6366F1" : "#E2E8F0"}`,
                          borderRadius: 8,
                          fontSize: 11,
                          fontWeight: 700,
                          cursor: "pointer",
                ...
                <div>
                  ...
                  <input
                    ...
                    min={1}
                    onChange={(e) =>
                      setEditQty(parseFloat(e.target.value) || 100)
                    }
                    style={{
                      width: "100%",
                      padding: "9px 12px",
