...
# PoseLandmark indices (BlazePose 33 keypoints)
class PoseLandmark:
    NOSE = 0
    LEFT_SHOULDER = 11
    RIGHT_SHOULDER = 12
...
# Các kết nối BlazePose để vẽ skeleton
POSE_CONNECTIONS = [
    (0, 1),
    (1, 2),
    (2, 3),
...
def _calculate_measurements_from_landmarks(
    ...
    """
    Tính toán số đo cơ thể từ danh sách tọa độ pixel.
    ...
    try:
        ...
        def get_pt(idx):
            return landmarks_px[idx]  # (x_px, y_px, visibility)
        ...
        try:
            ...
            leg_px = measurements["pixel_measurements"].get("leg_length", 0)
            ...
            measurements["classifications"] = {
                ...
                "somatotype": get_soma(s_px, h_px, leg_px, whr),
            }
        except Exception:
            measurements["classifications"] = {}
...
def _draw_skeleton(image, landmarks_px):
    """Vẽ bộ xương 33 điểm BlazePose lên ảnh bằng OpenCV."""
    SKEL_COLOR = (220, 220, 220)
    ...
    for a, b in POSE_CONNECTIONS:
        if a >= n or b >= n:
            continue
        ax, ay, av = landmarks_px[a]
        bx, by, bv = landmarks_px[b]
        if av < 0.3 or bv < 0.3:
            continue
        cv2.line(
            image, (int(ax), int(ay)), (int(bx), int(by)), SKEL_COLOR, 1, cv2.LINE_AA
        )

    for x, y, vis in landmarks_px:
        if vis < 0.3:
            continue
        cv2.circle(image, (int(x), int(y)), 3, DOT_COLOR, -1, cv2.LINE_AA)

    return image


def draw_measurements_on_image(image, measurements, landmarks_px):
    """Vẽ skeleton + các đường đo lên ảnh."""
    annotated_img = image.copy()

    # 1. Vẽ bộ xương 33 điểm (thay thế mp_drawing.draw_landmarks)
    if landmarks_px:
        _draw_skeleton(annotated_img, landmarks_px)

    # 2. Vẽ các đường đo kích thước
...
def analyze_pose_with_model(pose_model, image, known_height_cm=None):
    ...
    # Chuyển normalized landmarks sang pixel coordinates
